...
def plot_resource(df, value_vars, cluster_vars, cap_vars, y_label, title, filename):
    ...
    g = (
        ggplot()
        + geom_line(aes('timestamp', f'{y_label}_cluster', color='Legend', linetype='Legend'), size=1.5, data=cluster_pub)
        + geom_line(aes('timestamp', f'{y_label}_cap', color='Legend', linetype='Legend'), size=1.5, data=cap_pub)
        + geom_line(aes('timestamp', f'{y_label}_cluster', color='Legend', linetype='Legend'), size=1.5, data=cluster_priv)
        + geom_line(aes('timestamp', f'{y_label}_cap', color='Legend', linetype='Legend'), size=1.5, data=cap_priv)
