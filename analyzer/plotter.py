...
def plot_unschedulable(df: pd.DataFrame, output_dir: str):
    """
    Plot the count of ResourceBindings karmada-scheduler couldn't assign to
    *any* cluster over time (Scheduled=False) — workloads that never got
    propagated at all, so they never show up as Pending pods in the other
    plots. Only meaningful for baseline runs (see run_baseline.sh's polling
    loop); does nothing if df is empty.

    Args:
        df: DataFrame with columns: time_seconds, unschedulable_count
        output_dir: Directory to save the plot
    """
    if df.empty:
        return
    ...
    g = (
        ggplot(df, aes(x='time_seconds', y='unschedulable_count'))
        + geom_step(color='#B22222', size=1.2)
        + labs(title="Workloads unschedulable to any cluster over time",
               y="Unschedulable ResourceBindings", x="Time (seconds)")
        + scale_x_continuous(breaks=x_breaks)
        + scale_y_continuous(limits=(0, None))
        + theme_bw()
        + theme(legend_key=element_blank())
    ...
    timestamp = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    filename = f"unschedulable_{timestamp}.png"
    output_path = os.path.join(output_dir, filename)
    g.save(output_path, width=12, height=6, dpi=300)
