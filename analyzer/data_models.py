...
@dataclass
class UnschedulableSnapshot:
    """
    A poll of ResourceBindings karmada-scheduler couldn't assign to *any*
    cluster (status.conditions Scheduled=False), from run_baseline.sh's 30s
    polling loop. These workloads never get propagated at all, so they never
    show up as Pending pods in metrics.json — this is the only visibility
    into that failure mode. Only present for baseline runs today.
    """
    timestamp: int  # Unix timestamp
    count: int
...
class ProcessedSimulationData:
    ...
    # Total-scheduling-failure snapshots (baseline runs only — see UnschedulableSnapshot)
    unschedulable_snapshots: List[UnschedulableSnapshot] = field(default_factory=list)
