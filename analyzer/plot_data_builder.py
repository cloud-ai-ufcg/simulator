...
from data_models import ProcessedSimulationData, MigrationEvent, UnschedulableSnapshot


def build_unschedulable_dataframe(snapshots: List[UnschedulableSnapshot]) -> pd.DataFrame:
    """
    Build a DataFrame of ResourceBindings that failed to schedule to *any*
    cluster over time (see UnschedulableSnapshot / run_baseline.sh's polling
    loop). Empty (but correctly shaped) for runs with no such file, e.g. a
    normal AI-driven run.

    Returns DataFrame with columns: timestamp, time_seconds, unschedulable_count
    """
    if not snapshots:
        return pd.DataFrame(columns=['timestamp', 'time_seconds', 'unschedulable_count'])

    records = [
        {'timestamp': pd.to_datetime(s.timestamp, unit='s'), 'unschedulable_count': s.count}
