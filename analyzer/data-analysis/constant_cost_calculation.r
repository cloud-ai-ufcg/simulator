...
# --- 4. CALCULATING STATISTICS FOR CSV ---
stats_df <- df_all %>%
  mutate(
    # Total Cluster Capacity (Sum of the 2 environments)
    ...
    # % of Instantaneous Saturation
    saturation_pct = (total_used / total_capacity) * 100
  ) %>%
  group_by(model_name, architecture) %>%
  summarise(
    #1. Average Efficiency (% of the cluster that was actually used)
    avg_efficiency_pct = mean(saturation_pct, na.rm = TRUE),
    
    #2. Average Waste (Idle Capacity - What's Left Over)
    avg_waste_pct = 100 - mean(saturation_pct, na.rm = TRUE),
    
    #3. Peak Load (The maximum stress level the cluster was subjected to)
    peak_load_pct = max(saturation_pct, na.rm = TRUE),
    
    #4. Stability (Standard deviation - the lower the deviation, the more stable the consumption)
...
print(stats_df)
write_csv(stats_df, "tabela_eficiencia_constante.csv")