...
import sys
import pandas as pd
from data_loader import load_json_data, parse_migration_logs, load_unschedulable_data
...
def main():
    ...
    unschedulable_path = os.path.join(run_dir, "unschedulable_bindings.jsonl")
    ...
    run_dir = os.path.abspath(os.path.normpath(run_dir))
    run_name = os.path.basename(run_dir)

    # Write outputs alongside the run's own metrics.json/logs, under
    # simulator/data/output/<run_name>/, instead of a separate analyzer/output/ tree.
    ...
    try:
        ...
        # Try to load total-scheduling-failure data (baseline runs only)
        unschedulable_df = None
        if os.path.exists(unschedulable_path):
            unschedulable_df = load_unschedulable_data(unschedulable_path)
            print(f"✓ Loaded unschedulable-bindings data from {unschedulable_path}")

        # Step 2: Process data into structured models
        print("⚙️  Processing data...")
        processed_data = process_simulation_data(raw_data, migration_df, run_name, unschedulable_df)
        ...
        df_unschedulable = build_unschedulable_dataframe(processed_data.unschedulable_snapshots)
        ...
        if not df_unschedulable.empty:
            print("  - Unschedulable Workloads...")
            plot_unschedulable(df_unschedulable, plots_dir)

        # Step 5: Generate summary statistics
        print("📈 Generating summary statistics...")
        summarize(raw_data, summary_dir, df_migrations if not df_migrations.empty else None, unschedulable_df)
        ...
        if processed_data.unschedulable_snapshots:
            peak = max(s.count for s in processed_data.unschedulable_snapshots)
            print(f"   - Workloads unschedulable to any cluster (peak): {peak}")
