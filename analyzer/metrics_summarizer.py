...
def resolve_cluster_label(cluster_data) -> str:
...
def summarize_cost(list_of_cluster_info: list, interval_seconds: int = 30) -> list:
    ...
    for info in list_of_cluster_info:
        ...
        for cluster in info:
            cluster_label = resolve_cluster_label(cluster)
...
def summarize_resources(list_of_cluster_info: list, type_resource: str) -> list:
    ...
    for info in list_of_cluster_info:
        private_resource = 0.0
        public_resource = 0.0
        for cluster in info:
            label = resolve_cluster_label(cluster)
            if label == 'private':
                private_resource = cluster["cluster_load"][type_resource]
            elif label == 'public':
                public_resource = cluster["cluster_load"][type_resource]
...
def summarize_pending_pods(list_of_workloads: list, unschedulable_counts: list = None) -> list:
    """
    ...
    unschedulable_counts: optional list of per-poll counts of ResourceBindings
    karmada-scheduler couldn't assign to *any* cluster (baseline runs only —
    see load_unschedulable_data/run_baseline.sh's 30s poller). Those
    workloads never get a single Pod created anywhere, so they never show up
    in `list_of_workloads` at all; without folding them in here they'd
    silently vanish from this comparison instead of counting against the
    baseline the way a genuinely Pending pod counts against an AI-driven
    run. Each poll is folded in as its own sample (a workload is either
    fully affected or not — pod-level counts aren't known for bindings that
    were never propagated), so it only affects "percent_workloads", not the
    raw "pending_pods" pod count.
    ...
    for count in (unschedulable_counts or []):
        percent_workloads_list.append(1.0 if count > 0 else 0.0)
...
def summarize_time_with_pending(list_of_workloads: list, unschedulable_counts: list = None) -> list:
    """
    Calculate the number of periods (timestamps) with pending pods in any
    workload — or, for baseline runs, with any ResourceBinding unschedulable
    to any cluster at all (see summarize_pending_pods for why that matters).
    """
    ...
    for count in (unschedulable_counts or []):
        list_of_time_pending.append(1 if count > 0 else 0)
...
def summarize(raw_data, summary_dir, migration_df=None, unschedulable_df=None):
    """
    Receives a `raw_data` representing the metrics to summarize it.
    """
    grouped_data = grouping_elements(raw_data)
    unschedulable_counts = (
        unschedulable_df["unschedulable_count"].tolist()
        if unschedulable_df is not None and not unschedulable_df.empty
        else []
    )
    pending_data = summarize_pending_pods(grouped_data["workloads"], unschedulable_counts)
    ...
    data = {
        ...
        "time_with_pending": summarize_time_with_pending(grouped_data["workloads"], unschedulable_counts),
